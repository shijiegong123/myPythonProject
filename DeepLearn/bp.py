...
"""
map()函数功能是：map()是 Python 内置的高阶函数，它接收一个函数 f 和一个 list，
并通过把函数 f 依次作用在 list 的每个元素上，得到一个新的 list 并返回。
在python2中，map（）函数返回值是list类型，在python3中返回值改为map object类型，遍历前要转换成list才可以
"""
...
class NetWork(object):
    ...
    def __init__(self, layers):
        """初始化一个全连接神经网络
        参数@layers:一维数组,描述神经网络每层节点数
        如[2,2,2]输入层2个节点 隐藏层2个节点 输出层2个节点
        """
        self.connections=Connections()
        self.layers=[]                   #包含所有的神经网络层
    ...
    def predict(self,sample):
        ...
        # python3使用(list(map(lambda x: x[0], input_vecs)), labels)
...
def gradient_check(network, sample_feature, sample_label):
    ...
    network_error=lambda vec1, vec2: \
             0.5*reduce(lambda  a,b:a+b, list(map(lambda v:(v[0]-v[1])*(v[0]-v[1]),
                                             zip(vec1,vec2))))
    #获取网络在当前样本下每个连接的梯度
...
def correct_ratio(network):
    ...
    print('corrct_ration:%.2f%%'%(correct/256*100))
